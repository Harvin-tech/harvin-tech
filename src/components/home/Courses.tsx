'use client';
import React from 'react';
import Image from 'next/image';
import Link from 'next/link';
import { Swiper, SwiperSlide } from 'swiper/react';
import { Navigation } from 'swiper/modules';
import 'swiper/css';
import 'swiper/css/navigation';
import 'swiper/css/free-mode';
import { appContent } from '@/constants/variants';
import { Card, CardContent } from '../ui/card';
import { GoArrowRight } from 'react-icons/go';
import { useSearchParams } from 'next/navigation';
import { useDispatch } from 'react-redux';
import { useRouter } from 'next/navigation';
import { v4 as uuidv4 } from 'uuid';
import { setOneCourses } from '@/redux/courseSlice';
import { IoIosArrowBack, IoIosArrowForward } from 'react-icons/io';
import './swiper.css';

const dummyCourses = [
  {
    id: uuidv4(),
    courseId: '1',
    image: '/images/home/ai.png',
    title: 'AI & ML',
    instructor: 'Vikram Singh',
    description:
      'Harvinn Technologies’ AI & ML course is well-structured and beginner-friendly, covering key concepts like neural networks, Python, TensorFlow, and real-world projects. The hands-on approach and flexible learning schedule made the experience both practical and convenient.',
    rating: 5,
    reviewsCount: 205,
  },
  {
    id: uuidv4(),
    courseId: '2',
    image: '/images/home/iot.png',
    title: 'Data Science',
    instructor: 'Priya Sharma',
    description:
      'Harvinn Technologies’ data science course provided a perfect mix of theory and practical learning. From Python and SQL to machine learning and real-world projects, the content was comprehensive and industry-relevant.',
    rating: 5,
    reviewsCount: 128,
  },
  {
    id: uuidv4(),
    courseId: '3',
    image: '/images/home/web-dev.png',
    title: 'Full Stack Web Development',
    instructor: 'Rahul Mehta',
    description:
      'The full stack web development course was incredibly comprehensive, covering front-end, back-end, and database management. The hands-on projects and expert guidance helped me build real-world skills.',
    rating: 5,
    reviewsCount: 142,
  },
  {
    id: uuidv4(),
    courseId: '4',
    image: '/images/home/dm.png',
    title: 'Digital Marketing',
    instructor: 'Sneha Patel',
    description:
      'Harvinn Technologies’ digital marketing course provided in-depth knowledge of SEO, social media, analytics, and campaigns. The hands-on projects and expert guidance helped me build practical skills and boosted my confidence to excel in the field.',
    rating: 4.8,
    reviewsCount: 96,
  },
  {
    id: uuidv4(),
    courseId: '5',
    image: '/images/home/course.png',
    title: 'Pharmacovigilance',
    instructor: 'Amit Kumar',
    description:
      'The pharmacovigilance course at Harvinn Technologies was a transformative experience, covering everything from the fundamentals of drug safety to advanced regulatory guidelines and adverse event reporting systems.',
    rating: 4.9,
    reviewsCount: 74,
  },
  {
    id: uuidv4(),
    courseId: '6',
    image: '/images/home/mcode.png',
    title: 'Medical Coding',
    instructor: 'Anjali Gupta',
    description:
      'The medical coding course at Harvinn Technologies exceeded my expectations. The curriculum is comprehensive and up-to-date with the latest coding standards.',
    rating: 4.7,
    reviewsCount: 65,
  },
  {
    id: uuidv4(),
    courseId: '7',
    image: '/images/home/ai.png',
    title: 'AI & ML - Advanced',
    instructor: 'Karan Desai',
    description:
      'The AI & ML course I’ve taken is designed to keep learners engaged and motivated. The platform strikes the perfect balance between theory and practice.',
    rating: 5,
    reviewsCount: 88,
  },
  {
    id: uuidv4(),
    courseId: '9',
    image: '/images/home/data-science.png',
    title: 'Practical Data Science',
    instructor: 'Rohan Iyer',
    description:
      'The courses are packed with real-world examples and exercises. I’ve been able to apply what I learned immediately in my projects.',
    rating: 5,
    reviewsCount: 112,
  },
  {
    id: uuidv4(),
    courseId: '10',
    image: '/images/home/web-dev.png',
    title: 'Full Stack Development',
    instructor: 'Pooja Verma',
    description:
      'Enrolling in Harvinn Technologies has been one of the best decisions for my professional growth. The return on investment is evident in my career advancements.',
    rating: 5,
    reviewsCount: 99,
  },
  {
    id: uuidv4(),
    courseId: '11',
    image: '/images/home/dm.png',
    title: 'Business Analytics',
    instructor: 'Abhishek Reddy',
    description:
      'The instructors are experts in their fields and present concepts clearly. Their insights bring immense value to every lesson.',
    rating: 4.8,
    reviewsCount: 87,
  },
];

const Courses = () => {
  const searchParams = useSearchParams();
  const userId = searchParams.get('getUser_Id');
  const [loading, setLoading] = React.useState(false);
  const dispatch = useDispatch();

  const OPTIONS = {
    modules: [Navigation],
    spaceBetween: 24,
    freeMode: true,
    grabCursor: true,
    loop: false,
    navigation: {
      prevEl: '.swiper-button-prev',
      nextEl: '.swiper-button-next',
    },
    breakpoints: {
      320: {
        slidesPerView: 1,
        spaceBetween: 12,
      },
      640: {
        slidesPerView: 2,
        spaceBetween: 16,
      },
      768: {
        slidesPerView: 2,
        spaceBetween: 20,
      },
      1024: {
        slidesPerView: 3,
        spaceBetween: 24,
      },
      1280: {
        slidesPerView: 4,
        spaceBetween: 24,
      },
    },
  };

  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-lg text-primary">Loading courses...</div>
      </div>
    );
  }

  return (
    <div className="py-16 md:px-0  bg-gray-50">
      <div className={appContent({ className: '' })}>
        {/* Header Section */}
        <div className="max-w-3xl mx-auto text-center space-y-4 px-4 mb-8 md:mb-12">
          <h2 className="text-4xl md:text-5xl text-primary font-bold md:tracking-tight">
            Courses
          </h2>
          <p className="text-base md:text-lg text-foreground/80 max-w-2xl mx-auto tracking-tight">
            At Harvinn Technologies, we offer expert-led, flexible courses
            designed to help you grow, innovate, and succeed. Learn at your own
            pace, gain future-ready skills, and join a global community of
            learners. Your journey to success starts here!
          </p>
        </div>

        {/* Courses Carousel with Navigation and Gradients */}
        <div className="w-full h-full relative">
          {/* Left Gradient Overlay */}
          <div
            className="hidden md:block absolute left-0 top-0 bottom-8 w-36 z-10 pointer-events-none
                    bg-gradient-to-r from-background/70 via-background/10 to-transparent"
          />

          {/* Right Gradient Overlay */}
          <div
            className="hidden md:block absolute right-0 top-0 bottom-8 w-32 z-10 pointer-events-none
                    bg-gradient-to-l from-background/70 via-background/10 to-transparent "
          />

          {/* Navigation Buttons - Moved above gradient overlays */}
          <button
            className="swiper-button-prev absolute left-0 top-1/2 -translate-y-1/2 z-20 
                        p-6 flex items-center justify-center 
                       bg-white/90 hover:bg-white
                       rounded-full shadow-md hover:shadow-lg
                        transition-all duration-300
                       border border-border
                        !text-primary"
          ></button>

          <button
            className="swiper-button-next absolute right-0 top-1/2 -translate-y-1/2 z-20
                        p-6 flex items-center justify-center 
                       bg-white/90 hover:bg-white
                       rounded-full shadow-md hover:shadow-lg
                        transition-all duration-300
                       border border-border
                        !text-primary "
          ></button>

          <Swiper {...OPTIONS} className="!pb-8">
            {dummyCourses.map((course) => (
              <SwiperSlide key={course.id} className="!mx-1 flex items-stretch">
                <Link
                  href="/courses?preview=true"
                  onClick={() => dispatch(setOneCourses(course as any))}
                  className="block h-full w-full items-stretch"
                >
                  <Card className="h-full flex flex-col transition-all duration-300 hover:shadow-lg">
                    <CardContent className="p-0 h-full">
                      <div className="flex flex-col h-full">
                        {/* Image Container */}
                        <div className="relative aspect-video w-full overflow-hidden rounded-t-lg">
                          <Image
                            src={course.image}
                            alt={course.title}
                            layout="fill"
                            objectFit="cover"
                            className="transition-transform duration-300 hover:scale-105"
                          />
                        </div>

                        {/* Content Container */}
                        <div className="px-4 pb-4 flex flex-col gap-3 flex-grow">
                          <h3 className="font-semibold text-primary text-lg md:text-xl line-clamp-2">
                            {course.title}
                          </h3>
                          <p className="text-sm text-muted-foreground py-1 px-2 bg-muted w-fit rounded-full">
                            {course.instructor}
                          </p>
                          <p className="text-sm md:text-base text-foreground/80 line-clamp-2">
                            {course.description}
                          </p>

                          {/* Call to Action */}
                          <div className="mt-auto pt-4 flex items-center text-primary text-sm md:text-base font-medium">
                            <span>Learn More</span>
                            <GoArrowRight
                              className="ml-2 transition-transform duration-300 group-hover:translate-x-2"
                              size={18}
                            />
                          </div>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                </Link>
              </SwiperSlide>
            ))}
          </Swiper>
        </div>
      </div>
    </div>
  );
};

export default Courses;
